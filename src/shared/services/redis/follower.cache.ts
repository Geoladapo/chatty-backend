...
export const log: Logger = config.createLogger('followersCache');
...
export class FollowerCache extends BaseCache {
  ...
  public async updateBlockedUserPropInCache(key: string, prop: string, value: string, type: 'block' | 'unblock'): Promise<void> {
    try {
      ...
      const response: string = (await this.client.HGET(`users:${key}`, prop)) as string; // this return string of strings e.g "['123']"
